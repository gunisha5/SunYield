-- Fix RewardHistory table - Update existing records with proper dates
-- The date column already exists, so we just need to update the data
...
-- Add an index on the date column for better performance (will fail if already exists)
CREATE INDEX idx_reward_history_date ON reward_history(date);
