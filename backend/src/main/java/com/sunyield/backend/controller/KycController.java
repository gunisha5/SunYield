...
public class KycController {
    ...
    public ResponseEntity<?> submitKyc(
        ...
        if (existingKyc != null) {
            if (existingKyc.getStatus() == com.sunyield.backend.entity.KYCStatus.PENDING) {
                return ResponseEntity.badRequest().body("KYC already submitted and pending approval");
            }
            if (existingKyc.getStatus() == com.sunyield.backend.entity.KYCStatus.APPROVED) {
        ...
        try {
            ...
            kyc.setStatus(com.sunyield.backend.entity.KYCStatus.PENDING);
